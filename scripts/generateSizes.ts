...
function resize(directory, maxDimension, sourcePath) {
  const destinationPath = directory + '/' + sourcePath.substring(sourcePath.lastIndexOf('/') + 1);
  ...
  im.identify(['-format', '%wx%h', sourcePath], function(err, output) {
    if (err) {
      console.log("ERROR: " + destinationPath);
      throw err;
    }
